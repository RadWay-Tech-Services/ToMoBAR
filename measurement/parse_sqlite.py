...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_kernels", type=str, nargs='+', required=True, help="List of kernel names to analyze.")
    parser.add_argument("--end_to_end_runtime_ms", type=float, required=True, help="Total runtime in milliseconds.")
    parser.add_argument("--target_runtime_ms", type=float, required=True, help="Total runtime of measurement target in milliseconds.")
    parser.add_argument("--measurement_iteration_count", type=int, required=True, help="Number of measurement iterations.")
    ...
    for target_kernel in args.target_kernels:
        ...
        if target_kernel in kernel_duration_averages:
            kernel_duration_averages[target_kernel] /= actual_iteration_count
            kernel_duration_percentages[target_kernel] = kernel_duration_averages[target_kernel] / args.target_runtime_ms * 100

    cumulative_kernel_duration_percentages = f"{sum([x for x in kernel_duration_percentages.values()])} %"
    kernel_duration_percentages = { k: f"{v} %" for k, v in kernel_duration_percentages.items() }
