...
PD_TV_cupy_feri_orig_plotted = False
...
def prox_regul(self, X: cp.ndarray, _regularisation_: dict) -> cp.ndarray:
    ...
    global diff_plotted
    # if not diff_plotted:
    #     P_to_plot = (X_prox - X_prox_orig).get()
    #     plt.figure()
    #     plt.subplot(131)
    #     plt.imshow(P_to_plot[P_to_plot.shape[0] // 2, :, :])
    #     plt.colorbar()
    #     plt.title("diff, axial view")

    #     plt.subplot(132)
    #     plt.imshow(P_to_plot[:, P_to_plot.shape[1] // 2, :])
    #     plt.colorbar()
    #     plt.title("diff, coronal view")

    #     plt.subplot(133)
    #     plt.imshow(P_to_plot[:, :, P_to_plot.shape[2] // 2])
...
def PD_TV_cupy(
    ...
    theta = cp.float32(1.0)
    lt = cp.float32(tau / regularisation_parameter)
    enable_negativity_in_updates = cp.bool_(nonneg != 0)
    run_isotropic_kernel = cp.bool_(True) if methodTV == 0 else cp.bool_(False)
    ...
    if data.ndim == 3:
        ...
        grid_dims = (grid_x, grid_y, grid_z)
        module = load_cuda_module("primal_dual_for_total_variation")
        # module = load_cuda_module("primal_dual_for_total_variation", ["primal_dual_for_total_variation_2D<true>", "primal_dual_for_total_variation_2D<false>"])
        primal_dual_for_total_variation_3D = module.get_function(
            "primal_dual_for_total_variation_3D"
        )
    else:
        data3d = False
        ...
        module = load_cuda_module("primal_dual_for_total_variation")
        # module = load_cuda_module("primal_dual_for_total_variation", ["primal_dual_for_total_variation_2D<true>", "primal_dual_for_total_variation_2D<false>"])
        primal_dual_for_total_variation_2D = module.get_function(
            "primal_dual_for_total_variation_2D"
        )
    ...
    for _ in range(iterations):
        if data3d:
            params = (
                ...
                dy,
                dz,
                enable_negativity_in_updates,
                run_isotropic_kernel,
            )
            primal_dual_for_total_variation_3D(grid_dims, block_dims, params)
        else:
            params = (out, sigma, dx, dy, enable_negativity_in_updates)
            primal_dual_for_total_variation_2D(grid_dims, block_dims, params)
